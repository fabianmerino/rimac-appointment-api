...
export const handler = async (event: SQSEvent): Promise<void> => {
  console.log('Processing appointments for Chile (CL)');
  ...
  for (const record of event.Records) {
    try {
      ...
      // Create RDS record (specific business logic for Chile can be added here)
      ...
      // Save to RDS (MySQL) - could use different database/logic for CL
      await rdsRepository.saveAppointment(rdsRecord);
      console.log(`Appointment ${messageBody.appointmentId} saved to CL database`);
      ...
      console.error('Error processing appointment for CL:', error);
